...
- NativeWind with `@flama/design-system-mobile`
- TanStack Query, its cache persisted to MMKV
- i18next over `@flama/translations`
...
link scheme is `EXPO_PUBLIC_ADMIN_MOBILE_SCHEME` (default `flama-admin`), which
...
pnpm --filter @flama/admin-mobile prebuild     # regenerate ios/ and android/
pnpm --filter @flama/admin-mobile ios          # or: android
pnpm --filter @flama/admin-mobile dev          # Expo dev client on port 8083
pnpm --filter @flama/admin-mobile lint         # Biome + tsc --noEmit
pnpm --filter @flama/admin-mobile test         # Vitest
pnpm --filter @flama/admin-mobile arch         # dependency-cruiser
pnpm --filter @flama/admin-mobile build:dev    # EAS build (development profile)
```
...
- UI and native glue both Expo apps share — `FormField`, `useZodResolver`,
  ...
  `ExpoSecureStoreService` — are in `@flama/frontend-mobile`
