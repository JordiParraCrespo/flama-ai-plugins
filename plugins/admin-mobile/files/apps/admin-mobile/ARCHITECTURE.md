...
its `src/`; this app loads `@flama/frontend-admin` and would fail `pnpm arch`
for touching `@flama/frontend-consumer`. The two products meet only at a kernel
...
- `kit-through-its-entry` — `@flama/frontend-mobile` by its package name or one
...
- **`auth-client.ts`** — Better Auth with `expoClient({ scheme, storagePrefix:
  ...
  `EXPO_PUBLIC_ADMIN_MOBILE_SCHEME` (default `flama-admin`) and must match
