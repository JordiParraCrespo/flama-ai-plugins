import { FlamaProvider, MEMBER_LISTS_KEY, usersKeys } from '@flama/frontend-core/react';
...
function setup() {
  ...
  const app = fakeKernel({ [TOKENS.RolesService]: roles });
...
describe('mutations', () => {
  ...
  describe('assigning roles to a user', () => {
    ...
    it('invalidates every organization’s member list', async () => {
      ...
      await waitFor(() => expect(invalidated(invalidate, MEMBER_LISTS_KEY)).toBe(true));
    });
