...
- flama-ai now has two front doors — `apps/web` on 3000 and `apps/admin-web`
...
- Keep `src/` untouched except `paths.ts` (no change), `harness.ts` (add
  ...
  `organizationCounts` with flama's oracle counts).
...
- `qa/playwright.config.ts`: keep `workers: 1`, `retries: 0`, 1440×900 at 2×,
  ...
  `ADMIN_WEB_URL`.
- Rewrite `qa/README.md` for flama-ai (the "fixtures had to learn the hard way"
...
Accounts in `fixtures/accounts.ts`, all on `@qa.flama.dev` so `reset` can be
exhaustive and can never touch the platform seed's `@flama.dev`:
...
- `AUTH-02` password reset end to end via the mail sink; old password refused,
  link not replayable, **sessions revoked** (flama fix #114 — add a check).
