...
- Platform roles come from the seed: `superadmin@flama.dev`, `admin@flama.dev`,
  `user@flama.dev`. Everything the pack creates lives on `@qa.flama.dev`.
...
Steps: sign in as `superadmin@flama.dev` on `apps/web`; read the profile the
...
Steps: create `reset.member@qa.flama.dev` via sign-up; request a reset from
...
Steps: as the baseline owner, invite `invitee.admin@qa.flama.dev` as admin and
`invitee.member@qa.flama.dev` as member through the team screen; read both
...
Steps: register `newcomer@qa.flama.dev` on `/register`; follow wherever the
...
- `reset`: delete every `@qa.flama.dev` account whose address does not start
