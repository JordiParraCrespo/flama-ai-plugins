...
start_datastores() {
  ...
  su postgres -c "psql -tAc \"SELECT 1 FROM pg_roles WHERE rolname='flama'\"" | grep -q 1 || \
    su postgres -c "psql -c \"CREATE ROLE flama WITH LOGIN SUPERUSER PASSWORD 'flama'\"" >/dev/null
  su postgres -c "psql -tAc \"SELECT 1 FROM pg_database WHERE datname='flama'\"" | grep -q 1 || \
    su postgres -c "psql -c \"CREATE DATABASE flama OWNER flama\"" >/dev/null
...
up() {
  ...
  if curl -sf "$API_URL/api/v1/health" >/dev/null 2>&1 && mail_sink_ready; then
    ...
    (cd "$ROOT" && pnpm --filter @flama/api migration:run) >"$ARTIFACTS/migrate.log" 2>&1 \
      || die "migrations failed — see $ARTIFACTS/migrate.log"
    log 'starting the API'
    (cd "$ROOT" && nohup pnpm --filter @flama/api start >"$ARTIFACTS/api.log" 2>&1 &)
    wait_for "$API_URL/api/v1/health" 180 'API' || die "API did not come up — see $ARTIFACTS/api.log"
    log 'seeding the platform accounts'
    (cd "$ROOT" && pnpm --filter @flama/api seed) >>"$ARTIFACTS/api.log" 2>&1 || die 'seed failed'
  fi
  ...
  if curl -sf "$WEB_URL" >/dev/null 2>&1; then
    ...
    (cd "$ROOT" && nohup pnpm --filter @flama/web dev >"$ARTIFACTS/web.log" 2>&1 &)
  ...
  if curl -sf "$ADMIN_WEB_URL" >/dev/null 2>&1; then
    ...
    (cd "$ROOT" && nohup pnpm --filter @flama/admin-web dev >"$ARTIFACTS/admin-web.log" 2>&1 &)
