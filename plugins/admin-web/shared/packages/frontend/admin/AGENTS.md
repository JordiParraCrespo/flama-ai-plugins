# @flama/frontend-admin — Agent Instructions
...
- A permission or role rule the API also enforces → `@flama/shared`, not a
...
pnpm --filter @flama/frontend-admin build   # the apps import dist/
```

## Patterns agents get wrong

- Calling `useFlamaApp()` in an admin hook. It resolves kernel services only;
  an admin hook reads `useAdminApp().roles`.
- Importing `@flama/frontend-consumer` to invalidate a member list.
  `products-never-meet` fails: invalidate `MEMBER_LISTS_KEY` from
  `@flama/frontend-core/react`, as `useAssignUserRoles` does.
