...
[Feature flags](./feature-flags.md). PostHog's own flag loading is switched off,
so an ad blocker that eats the analytics SDK cannot change what the product
shows.
...
dropped. On web the SDK is also dropped from the bundle
