...
A product package imports the kernel, `@flama/shared` and `@flama/api-client`,
...
`apps/<app>/src/lib/flama.ts` is the composition root, and the modules it loads
...
`FlamaApp` binds the kernel's modules and then whatever `modules` the app
passes. `FlamaProvider` puts the app in context and `useFlamaApp()` reads it.
